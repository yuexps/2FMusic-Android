...
### 2.5 收藏夹与歌单交互规范
*   **右上角无新建按钮**：为保持收藏夹主页面清爽，右上角不设置单独的新建歌单按钮。
*   **入口下沉至选择弹窗**：新建歌单的入口以“创建新收藏夹”按钮形式嵌入在“移动音乐至 / 添加音乐至”选择对话框的底部（替换原“取消”按钮，通过点击对话框外部即可完成取消/Dismiss）。
*   **响应式列表更新**：点击“创建新收藏夹”会直接唤起“新建歌单”对话框。在新建成功后，由于 `playlists` 数据流的响应式推送，底层的目标歌单选择弹窗列表会实时自动刷新并渲染出最新创建的歌单，用户可直接在列表中点击以继续完成移入/添加操作。
*   **空状态文本居中**：在选择或列表弹窗中，若待选列表为空，所呈现的提示文本（如“暂无歌单”、“暂无可选择的其它歌单”等）应统一设置 `fillMaxWidth()` 和 `textAlign = TextAlign.Center` 以实现水平居中对齐。
*   **历史记录歌单菜单特异化**：为了防止在“播放历史”歌单中出现有歧义的“移动至其它歌单”等菜单项，必须通过将 `isHistory` 参数设为 `currentPlaylist.id == "history"` 来限制其菜单内容，使其仅展示“删除记录”。
*   **全局播放栏遮挡协同**：进入批量管理模式时，应通过 `onBatchModeChange` 回调将状态广播至全局，从而安全地隐藏常驻迷你悬浮播放器。隐藏后，批量工具栏（`FloatingToolbar`）统一使用无底边距（`bottom = 0`）的通栏沉浸浮动设计，使收藏夹详情页与音乐库主页的批量管理体验完全一致。

### 2.6 全屏播放器交互与返回拦截规范
*   **统一滑出折叠**：全屏播放器（`PlayerScreen`）的折叠关闭动作（通过下拉手势、顶部向下箭头收起）必须统一调用 `handleDismiss`，执行长达 `280ms` 的向下平滑滑出动画，再回调 `onClose()` 卸载页面。
*   **物理返回拦截**：必须在 `PlayerScreen` 内使用 `utils.BackHandler(enabled = true)` 拦截系统的物理或手势返回键，并在事件中自动触发 `handleDismiss()` 退出，严禁因未拦截返回键导致无过度动画、或由于系统默认返回直接导致 Activity 整个退出。
